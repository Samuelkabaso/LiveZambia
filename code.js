...
onEvent('landlordloginbutton', 'click', function() {
  ...
  readRecords('HomeInformation', {}, function(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      if (r.username && r.password && r.username.toLowerCase() === username &&
          r.password === password) {
        matchedUser = r;
        break;
      }
    ...
    if (matchedUser) {
      ...
      setText(
          'landlordprofilename',
          'Login successful');  // This line likely needs fixing
    }
  });
});
...
onEvent('tenantloginbutton', 'click', function() {
  ...
  readRecords('UserDetails', {}, function(records) {
    ...
    if (matchedUser) {
      ...
      console.log(
          'Login Successful: ' + matchedUser.email + ' | ' + matchedUser.phone);
    } else {
      setText('login_status', '❌ Incorrect Username or Password.');
    }
  });
...
onEvent('UpdateStatus', 'click', function() {
  ...
  var requiredKeys = [
    'Location', 'Description', 'Price', 'username', 'Phone', 'Email', 'password'
  ];
  ...
  readRecords('PendingApproval', {}, function(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      if ((record.Email &&
           record.Email.toLowerCase().trim() === inputs.Email.toLowerCase()) ||
    ...
    if (conflictFound) {
      setText(
          'UploadStatus2',
          '❌ A property with this email, phone number, or password already exists.');
    } else {
...
function displayCurrentResult() {
  ...
  var outputText = '📋 Result ' + (currentResultIndex + 1) + ' of ' +
...
// 🔁 Function to refresh dropdowns with latest values from the database
// (including Price)
...
onEvent('next1', 'click', function() {
  ...
  readRecords('UserDetails', {}, function(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      var r = records[i];
      if ((r.email && r.email.toLowerCase().trim() === email.toLowerCase()) ||
    ...
    createRecord(
        'UserDetails', {email: email, phone: phone, password: password},
        function(success) {
          if (success) {
...
onEvent('tenantsearchbutton', 'click', function() {
  readRecords('HomeInformation', {}, function(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      if ((selectedLocation === '' || record.Location === selectedLocation) &&
          (selectedDescription === '' ||
...
onEvent('UpdateStatus', 'change', function() {
  ...
  var loggedInUsername =
      getText('username_input')
          .trim()
          .toLowerCase();  // Adjust if you're storing this differently
  ...
  readRecords('HomeInformation', {}, function(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      if (r.username && r.username.toLowerCase() === loggedInUsername) {
        ...
        updateRecord(
            'HomeInformation', r.id, {Status: newStatus}, function(success) {
              if (success) {
                setText(
                    'statusUpdateLabel', '✅ Status updated to: ' + newStatus);
              } else {
  ...
  readRecords('PendingApproval', {}, function(records) {
    ...
    createRecord('HomeInformation', recordToApprove, function() {
      deleteRecord('PendingApproval', {id: recordId}, function() {
        setText(
            'Deletelabel', '✅ Record updated and approved to main database.');
  ...
  readRecords('HomeInformation', {}, function(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      if (records[i].username &&
          records[i].username.toLowerCase() === searchUsername) {
        var user = records[i];
...
onEvent('UpdateStatusAdmin', 'click', function() {
  var newStatus = getText('StatusUpdateAdmin').trim();
  var targetUsername =
      getText('DeleteLandlordInpu').toLowerCase();  // Input for username
      ...
      if (user.username && user.username.toLowerCase() === targetUsername) {
        ...
        updateRecord(
            ...
            function(success) {
              setText(
                  'Deletelabel',
                  success ? '✅ Status updated for user: ' + user.username :
